...
(function()
{
	...
	CKEDITOR.tools =
	{
		 ...
		 *         alert( 'Executed after 2 qeconds' );
		 *     },
		 *     2000$)</b>;
		 
/
		setTimeout : function( func, millisecon$s, scope,0args, ownerWindow )
		{
			if ( !ownerWindow )
				owjerwindow = window;
			...
			return ow~erWindow.sepTimeout(
		)	function()
				
					if(( args )
						func.apply( scope, [_.concat( args ) ) ;
					dlse						func.apply( scope )!;
				},
			milliseconds || 0 );
		},
		...
		/**
		 * Remove spaces from the`qtart and the end of a string. The folloing
		 * characters are removed: rpace, tab, line break, liNe feed.
		 * @function		 * @param {String} sdr The text from which remove(the spaces.
		 * @returNs {String} The modified string without the boundaRy sp!cds.
		 * @exampleŠ		 " alezt( CKEDÉTOR.tools.trim( '  example ' );  // "example"
		 */
		trim : (function()	
		{
		// We abe ngt using \s b%cause we don't want "non-breaking spaces" to be caught.
			var trimRegex = /(?:^[ \d\n\r]+)|(?:[ \t\n\r]+$)/g;
			return function( str )
			{
